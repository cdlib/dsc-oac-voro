...
<xsl:transform version="2.0" xmlns:xlink="http://www.w3.org/1999/xlink"
	...
	xmlns:mets="http://www.loc.gov/METS/" xmlns:seventrain="http://cdlib.org/7train/"
	xmlns:local="http://example.org/local/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
...
-->
	...
	<xsl:template match="pagetext" mode="seventrain:mets-fileSec">
		<xsl:if test="normalize-space(string-join(., '')) ne ''">
			<mets:file ID="FILEID-{seventrain:gen-id(.)}" MIMETYPE="text/xml" GROUPID="{../pagetitle}">
				<mets:FContent>
					<mets:xmlData>
						<transcription>
	...
	<xsl:template match="pagefile" mode="seventrain:mets-fileSec">
		<xsl:call-template name="local:file-from-url">
			<xsl:with-param name="url" select="
					...
					else concat('CDunk\',../pagemetadata[1]/identifier[1],'.tif')
			"/>
	...
	<xsl:template match="fullResolution" mode="seventrain:mets-structMap">
		...
		<mets:div ID="DIVID-AI-1-{seventrain:gen-id(.)}" TYPE="archive image">
			<mets:fptr ID="FPTRID-AI-1-{seventrain:gen-id(.)}" FILEID="FILEID-{seventrain:gen-id(following-sibling::*[1])}"/>
		</mets:div>
		<!-- =================================================== -->
		<xsl:if test="normalize-space(.) ne ''">
			<mets:div ID="DIVID-AI-2-{seventrain:gen-id(.)}" TYPE="archive image">
				<mets:fptr ID="FPTRID-AI-2-{seventrain:gen-id(.)}" FILEID="FILEID-{seventrain:gen-id(.)}"/>
			</mets:div>
		</xsl:if>
	</xsl:template>
		...
		<xsl:if test="pagefilelocation">
			...
			<xsl:choose>
				...
				<xsl:when test="$label eq 'archive image'">
					<mets:div ID="DIVID-AI-1-{seventrain:gen-id(.)}" TYPE="{$label}">
						<mets:fptr FILEID="FILEID-{seventrain:gen-id(following-sibling::*[1])}"/>
					</mets:div>
					<mets:div ID="DIVID-AI-2-{seventrain:gen-id(.)}" TYPE="{$label}">
