...
<!-- 
 ...
 <xsl:transform
  ...
  <xsl:function name="seventrain:gen-id" as="xs:string">
    <xsl:param name="node-needing-id"/>
    <xsl:for-each select="$node-needing-id"><xsl:number count="*" level="multiple"/></xsl:for-each>
  </xsl:function>
  ...
  <!-- Override this template to change the ID attribute of the <mets>
       element. -->
  <xsl:template match="*[node-name(.)=$base-element]" mode="seventrain:mets.ID">METSID-<xsl:value-of select="seventrain:gen-id(.)"/></xsl:template>
  ...
  <!-- Override this template to change the ID attribute of the
       <metsHdr>. -->
  <xsl:template match="*[node-name(.)=$base-element]" 
		mode="seventrain:mets-metsHdr.ID">METSHDRID-<xsl:value-of select="seventrain:gen-id(.)"/></xsl:template>
